...
class AlexNet(nn.Module):
    ...
    def __init__(self, num_classes: int = 1000) -> None:
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=11, stride=1, padding=3), # kernel_size: 11->7, stride: 4->1
            # nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=3), # kernel_size: 11->7, stride: 4->1
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
...
def alexnet(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> AlexNet:
    ...
    if pretrained:
        ...
        for name, param in state_dict.items():
            ...
            #     with torch.no_grad():
            #         model_dict[name] = state_dict[name].clone().resize_(model_dict[name].size())
            #     # continue
            # elif name in ['features.0.bias']:
            #     continue
                # continue
